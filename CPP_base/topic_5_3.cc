...
int input_array_size(int *n, int *m) {
  ...
  if (*n <= 0 || *m <= 0 || *n > MAX_SIZE || *m > MAX_SIZE) {
    cout << "Ошибка: размеры массива должны быть положительными и не более "
         << MAX_SIZE << "." << endl;
    return 1;
  }
  return 0;
...
void input_array(int **a, int n, int m) {
  ...
  cout << "Введите значения для массива в " << n << " строк и " << m
       << " столбцов:" << endl;
  for (int i = 0; i < n; i++)
...
void rm_min_col(int **a, int **result, int n, int m, int min_col) {
  for (int i = 0; i < n; ++i) result[i] = new int[m - 1];
  for (int i = 0; i < n; i++) {
